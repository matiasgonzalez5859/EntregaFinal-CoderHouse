...
// función renderizar filtro productos desde navbar bovinos
function fitrarPorBovino () {
    let seleccion = document.getElementById('filtro_Bovino').addEventListener('click', ()=> {   
        console.log(seleccion)     
    fetch('./js/stock.json') //=> ubicacion de stock productos formateado a json
        .then(res=> res.json())
        .then(data=>{ 
            filtroTipo = data.filter(prod => prod.tipo === "Bovino");
renderizarCards(filtroTipo)})})
}


// función renderizar filtro productos desde navbar equinos
function fitrarPorCaballos () {
    let seleccion = document.getElementById('filtro_Caballos').addEventListener('click', ()=> {   
        console.log(seleccion)     
    fetch('./js/stock.json') //=> ubicacion de stock productos formateado a json
        .then(res=> res.json())
        .then(data=>{ 
            filtroTipo = data.filter(prod => prod.tipo === "Caballos");
renderizarCards(filtroTipo)})})
}

// función renderizar filtro productos desde navbar Granos
function fitrarPorGranos () {
    let seleccion = document.getElementById('filtro_Granos').addEventListener('click', ()=> {   
        console.log(seleccion)     
    fetch('./js/stock.json') //=> ubicacion de stock productos formateado a json
        .then(res=> res.json())
        .then(data=>{ 
            filtroTipo = data.filter(prod => prod.tipo === "Granos");
renderizarCards(filtroTipo)})})
...
function mostrarTotal() {
    ...
    div.innerHTML = `<h3 class="total_carrito">Total carrito: $${precioTotal}</h3>
                     <button id="finalizar_compra" class="button_card" type="button">
                        <a href="./pages/compra.html" class="button_card">Finalizar compra!</a>
                     </button>
